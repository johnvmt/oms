...
Utils.objectPublicFunctions = function(object) {
...
/**
 *
 * @param {function[]} queueFunctions - Array of functions
 * @param {object[]} queueFunctionArgs - Arguments to pass to the functions
 * @param {function} queueCompleteCallback - Callback called when queue completes
 * @param {function} queueNoMatchCallback - Callback called if queue completes without exiting
 * @returns {boolean}
 */
Utils.callbackQueue = function() {
	...
	parsedArgs.options = Utils.objectMerge({expand: true}, parsedArgs.options);
	...
	function callbackQueueNext() {
		...
		if(typeof parsedArgs.queueFunctions[index] === 'function') {
			if(Array.isArray(parsedArgs.queueFunctionArgs) && !parsedArgs.options.expand) // pass arguments to function as array// TODO add expand option
				parsedArgs.queueFunctions[index](parsedArgs.queueFunctionArgs, callbackQueueNext);
				...
				parsedArgs.queueFunctions[index].apply(this, parsedArgs.queueFunctionArgs.concat([callbackQueueNext]));
			else // no arguments to pass
				parsedArgs.queueFunctions[index](callbackQueueNext);
		}
		else if(typeof parsedArgs.queueNoMatchCallback == 'function')
			parsedArgs.queueNoMatchCallback.apply(this, parsedArgs.queueFunctionArgs);
	}
	callbackQueueNext();
};
